...
const form = useForm({
    ...
    cpm: props.template?.cpm || '',
    max_budget: props.template?.max_budget || '',
    max_reward_per_clipper: props.template?.max_reward_per_clipper || '',
    duration_days: props.template?.duration_days || '',
    is_public: props.template?.is_public || false,
});

const submit = () => {
    form.put(route('clipper.campaign-templates.update', props.template.id), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head title="Edit Campaign Template" />
