...
<div class="py-8 sm:py-12 bg-gray-50 min-h-screen">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-6">
            <a href="{{ route('public.profile.show', $seller->username) }}" 
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div class="w-16 h-16 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center shadow-lg overflow-hidden">
                        @if($seller->avatar)
                            @if(str_starts_with($seller->avatar, 'http'))
                                <img src="{{ $seller->avatar }}" alt="{{ $seller->name }}" class="w-16 h-16 rounded-full object-cover">
                            @else
                                <img src="{{ Storage::url($seller->avatar) }}" alt="{{ $seller->name }}" class="w-16 h-16 rounded-full object-cover">
                            @endif
                        @else
                            <span class="text-2xl font-bold text-white">{{ strtoupper(substr($seller->name, 0, 1)) }}</span>
                        @endif
                    </div>
                    
                    <div class="flex-1">
                        <h1 class="text-2xl font-bold text-gray-900 mb-1">AI Chat: {{ $seller->name }}</h1>
                        <p class="text-sm text-gray-600">
                            Ask questions about {{ $seller->name }}'s notes. AI will answer based on {{ $publicNotes->count() }} public notes.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200" id="ai-chat-container">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 bg-gray-50 rounded-lg p-4">
                            <p class="text-sm text-gray-700">
                                👋 Hi! I'm an AI assistant that can answer questions about <strong>{{ $seller->name }}</strong>'s notes. 
                                I have access to {{ $publicNotes->count() }} public notes from this seller.
                            </p>
                            <p class="text-xs text-gray-500 mt-2">
                                Try asking: "What topics does {{ $seller->name }} write about?" or "Tell me about the latest notes"
                            </p>
                        </div>
                    </div>
                @endif
            ...
            <div class="border-t border-gray-200 p-4">
                <form id="chat-form" class="flex gap-2">
                    ...
                    <input type="text" 
                           id="question-input" 
                           placeholder="Ask a question about {{ $seller->name }}'s notes..." 
        ...
        @if($publicNotes->count() > 0)
            <div class="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <p class="text-sm text-gray-600">
                    <strong>{{ $publicNotes->count() }}</strong> public notes available for AI context.
                    @if($publicNotes->count() > 10)
                        Showing first 10 notes, AI can access all {{ $publicNotes->count() }} notes.
                    @endif
                </p>
            </div>
        @endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    chatForm.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('{{ route("public.profile.ai-chat.ask", $seller->username) }}', {
                method: 'POST',
                headers: {
        ...
        if (type === 'user') {
            ...
            if (referencedNotes && referencedNotes.length > 0) {
                ...
                referencedNotes.forEach(note => {
                    notesHtml += `<a href="{{ route('marketplace.show', ':id') }}" class="inline-flex items-center px-2 py-1 rounded text-xs bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors">${escapeHtml(note.title)}</a>`.replace(':id', note.id);
                });
                notesHtml += '</div></div>';
            }
