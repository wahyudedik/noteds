            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['conversion_rate'], 2) }}%</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <div class="mb-4">
                        ...
                        <p class="text-3xl font-bold text-gray-900">{{ number_format($conversionData['overall_conversion_rate'], 2) }}%</p>
                        <p class="text-xs text-gray-500 mt-1">{{ number_format($conversionData['total_views']) }} views → {{ number_format($conversionData['total_purchases']) }} purchases</p>
                    </div>
                </div>
                <div>
                    ...
                    <div class="space-y-3">
                        @forelse($conversionData['by_note']->take(5) as $note)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-900">{{ \Illuminate\Support\Str::limit($note['note_title'], 40) }}</p>
                                    <p class="text-xs text-gray-600">{{ $note['views'] }} views, {{ $note['purchases'] }} purchases</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-semibold text-green-600">{{ number_format($note['conversion_rate'], 2) }}%</p>
                                </div>
                            </div>
                        @empty
                ...
                <div class="space-y-4">
                    @foreach($trafficSources as $source)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                            <div class="flex-1">
                                <p class="text-sm font-semibold text-gray-900 capitalize">{{ $source['source'] ?: 'Unknown' }}</p>
                                <p class="text-xs text-gray-600">{{ number_format($source['views']) }} views</p>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-semibold text-gray-900">{{ number_format($source['purchases']) }} purchases</p>
                                <p class="text-xs text-green-600">{{ number_format($source['conversion_rate'], 2) }}% conversion</p>
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
            ...
            <div class="space-y-4">
                @foreach(array_slice($geographicData, 0, 10) as $geo)
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-gray-900">{{ $geo['country_name'] }} ({{ $geo['country_code'] }})</p>
                            <p class="text-xs text-gray-600">{{ number_format($geo['views']) }} views</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-semibold text-gray-900">{{ number_format($geo['purchases']) }} purchases</p>
                            <p class="text-xs text-green-600">{{ number_format($geo['conversion_rate'], 2) }}% conversion</p>
                        </div>
                    </div>
                @endforeach
            </div>
                    ...
                    @foreach($abTests as $test)
                        <div class="p-4 bg-gray-50 rounded-lg">
                            <div class="flex items-center justify-between mb-2">
                                <h3 class="text-sm font-semibold text-gray-900">{{ $test->note->title ?? 'Unknown Note' }}</h3>
                                <span class="px-2 py-1 text-xs font-medium rounded-full {{ $test->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                    {{ ucfirst($test->status) }}
                                </span>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-sm font-semibold">{{ number_format($test->variant_a_conversion_rate ?? 0, 2) }}%</p>
                                    <p class="text-xs text-gray-500">{{ $test->variant_a_views }} views, {{ $test->variant_a_purchases }} purchases</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-600">Variant B</p>
                                    <p class="text-sm font-semibold">{{ number_format($test->variant_b_conversion_rate ?? 0, 2) }}%</p>
                                    <p class="text-xs text-gray-500">{{ $test->variant_b_views }} views, {{ $test->variant_b_purchases }} purchases</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
