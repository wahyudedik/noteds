...
@section('title', __('messages.admin_subscriptions'))
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-2">
                <div class="flex items-center">
                    ...
                    <h1 class="text-3xl font-bold text-gray-900">{{ __('messages.subscription_management') }}</h1>
                </div>
            ...
            <p class="mt-2 text-base text-gray-600">{{ __('messages.manage_premium_subscription_requests') }}</p>
        </div>
        ...
        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6 mb-8">
            <form method="GET" action="{{ route('admin.subscriptions.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    <label for="plan" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.plan') }}</label>
                    <select name="plan" id="plan" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-200">
                        ...
                        <option value="basic" {{ request('plan') === 'basic' ? 'selected' : '' }}>{{ __('messages.basic') }}</option>
                        <option value="premium" {{ request('plan') === 'premium' ? 'selected' : '' }}>{{ __('messages.premium') }}</option>
                    </select>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.plan') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.status') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.expires') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.submitted') }}</th>
        ...
        @else
            <div class="bg-white overflow-hidden shadow-sm rounded-lg border border-gray-200 text-center py-16 px-6">
                ...
                <h3 class="mt-4 text-lg font-medium text-gray-900">{{ __('messages.no_subscriptions_found') }}</h3>
                <p class="mt-2 text-sm text-gray-500">{{ __('messages.no_subscriptions_matching_filters') }}</p>
            </div>
        @endif
    </div>
</div>
