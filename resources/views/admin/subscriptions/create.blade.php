...
@section('title', __('messages.admin_create_subscription'))
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <div class="flex items-center mb-2">
                ...
                <h1 class="text-3xl font-bold text-gray-900">{{ __('messages.create_subscription') }}</h1>
            </div>
            <p class="mt-2 text-base text-gray-600">{{ __('messages.manually_create_subscription') }}</p>
        </div>
        ...
        <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
            <form action="{{ route('admin.subscriptions.store') }}" method="POST" class="p-6">
                ...
                <!-- User Selection -->
                <div class="mb-6">
                    <label for="user_id" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.select_user') }}</label>
                ...
                <!-- Plan Selection -->
                <div class="mb-6">
                    <label for="plan" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.plan_type') }}</label>
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative flex cursor-pointer rounded-lg border-2 p-4 focus:outline-none {{ old('plan') === 'basic' ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-300' }}">
                            ...
                            <div class="flex flex-1">
                                <div class="flex flex-col">
                                    <span class="block text-sm font-medium text-gray-900">{{ __('messages.basic') }}</span>
                                    <span class="mt-1 flex items-center text-sm text-gray-500">10 {{ __('messages.notes_total') }}</span>
                                </div>
                            </div>
                        ...
                        <label class="relative flex cursor-pointer rounded-lg border-2 p-4 focus:outline-none {{ old('plan') === 'premium' ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-300' }}">
                            ...
                            <div class="flex flex-1">
                                <div class="flex flex-col">
                                    <span class="block text-sm font-medium text-gray-900">{{ __('messages.premium') }}</span>
                                    <span class="mt-1 flex items-center text-sm text-gray-500">{{ __('messages.unlimited_notes') }}</span>
                                </div>
                            </div>
                ...
                <!-- Expiration Date -->
                <div class="mb-6">
                    <label for="expired_at" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.expiration_date') }}</label>
                    ...
                    @error('expired_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">{{ __('messages.select_when_expires') }}</p>
                </div>

                <!-- Admin Notes -->
                <div class="mb-6">
                    <label for="admin_notes" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.admin_notes_optional') }}</label>
                    <textarea name="admin_notes" id="admin_notes" rows="3"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-200"
                        :placeholder="__('messages.add_notes_context')">{{ old('admin_notes') }}</textarea>
                    @error('admin_notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
