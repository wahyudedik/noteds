            ...
            <form action="{{ route('admin.affiliate-settings.update') }}" method="POST" class="space-y-8 p-6">
                ...
                <div class="border-b border-gray-200 pb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @for ($i = 1; $i <= 6; $i++)
                            @php
                                $threshold = $settings['affiliate_conversion_threshold_' . ($i - 1)] ?? ($i === 1 ? 0 : null);
                                $nextThreshold = $settings['affiliate_conversion_threshold_' . $i] ?? null;
                                $range = $i === 1 ? '0 - ' . ($nextThreshold - 1) : ($threshold . ' - ' . ($nextThreshold ? $nextThreshold - 1 : '+'));
                            @endphp
                            <div class="border border-gray-200 rounded-lg p-4">
                                <div class="flex items-center justify-between mb-4">
                                    <label for="affiliate_commission_tier_{{ $i }}" class="block text-sm font-medium text-gray-900">
                                        {{ __('affiliate.tier') }} {{ $i }}
                                        <span class="text-xs text-gray-500 ml-2">({{ $range }} conversions)</span>
                                    </label>
                                </div>
                                <div class="flex items-center gap-2">
                                    <input 
                                        type="number" 
                                        name="affiliate_commission_tier_{{ $i }}" 
                                        id="affiliate_commission_tier_{{ $i }}"
                                        step="0.01"
                                        min="0"
                                        max="100"
                ...
                <div class="border-b border-gray-200 pb-8">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('affiliate.conversion_thresholds') }}</h3>
                    <p class="text-sm text-gray-600 mb-6">{{ __('affiliate.conversion_thresholds_description') }}</p>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @for ($i = 1; $i <= 6; $i++)
                            <div class="border border-gray-200 rounded-lg p-4">
                                <label for="affiliate_conversion_threshold_{{ $i }}" class="block text-sm font-medium text-gray-900 mb-2">
                                    {{ __('affiliate.threshold') }} {{ $i }}
                                    <span class="text-xs text-gray-500 ml-2">(to reach Tier {{ $i + 1 }})</span>
                                </label>
                                <input 
                                    type="number" 
                                    name="affiliate_conversion_threshold_{{ $i }}" 
                                    id="affiliate_conversion_threshold_{{ $i }}"
                                    min="1"
                ...
                <div class="border-b border-gray-200 pb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="border border-gray-200 rounded-lg p-4">
                            <label for="affiliate_min_payout_amount" class="block text-sm font-medium text-gray-900 mb-2">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <input 
                                    type="number" 
                                    name="affiliate_min_payout_amount" 
                                    id="affiliate_min_payout_amount"
                                    step="0.01"
                                    min="0.01"
                        ...
                        <div class="border border-gray-200 rounded-lg p-4">
                            ...
                            <input 
                                type="number" 
                                name="affiliate_payout_day" 
                                id="affiliate_payout_day"
                                min="1"
                                max="31"
                                value="{{ $settings['affiliate_payout_day'] }}"
