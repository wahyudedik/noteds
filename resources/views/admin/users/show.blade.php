        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-4">
                <a href="{{ route('admin.users.index') }}"
                    class="text-blue-600 hover:text-blue-800">{{ __('messages.back_to_users') }}</a>
            </div>
                    ...
                    <div class="space-y-2">
                        <p>
                            ...
                            @if ($user->verification_status === 'approved')
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">{{ __('messages.approved') }}</span>
                            @elseif($user->verification_status === 'rejected')
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-700">{{ __('messages.rejected') }}</span>
                            @else
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700">{{ __('messages.pending') }}</span>
                            @endif
                        </p>
                        @if ($user->agreement_accepted_at)
                            <p><strong>{{ __('messages.agreement_label') }}</strong> {{ __('messages.accepted') }}
                                {{ $user->agreement_accepted_at->format('d M Y H:i') }}
                                ({{ $user->agreement_version ?? 'v1' }})</p>
                        @endif
                        @if ($user->verification_reviewed_at)
                            <p><strong>{{ __('messages.reviewed_label') }}</strong>
                                {{ $user->verification_reviewed_at->format('d M Y H:i') }}</p>
                        @endif
                        @if ($user->verification_notes)
                            <p class="text-sm"><strong>{{ __('messages.notes_label') }}</strong>
                                {{ $user->verification_notes }}</p>
                        @endif
                    </div>
                    <div class="space-y-3">
                        <div class="flex gap-3">
                            <a href="{{ $user->ktp_path ? route('admin.users.download-doc', ['user' => $user->id, 'type' => 'ktp']) : '#' }}"
                                class="px-3 py-2 text-xs rounded-md {{ $user->ktp_path ? 'bg-slate-100 hover:bg-slate-200 text-slate-800' : 'bg-slate-50 text-slate-400 cursor-not-allowed' }}"
                                @if (!$user->ktp_path) aria-disabled="true" @endif>{{ __('messages.download_ktp') }}</a>
                            <a href="{{ $user->selfie_path ? route('admin.users.download-doc', ['user' => $user->id, 'type' => 'selfie']) : '#' }}"
                                class="px-3 py-2 text-xs rounded-md {{ $user->selfie_path ? 'bg-slate-100 hover:bg-slate-200 text-slate-800' : 'bg-slate-50 text-slate-400 cursor-not-allowed' }}"
                                @if (!$user->selfie_path) aria-disabled="true" @endif>{{ __('messages.download_selfie') }}</a>
                        </div>
                        <div class="flex flex-wrap gap-3">
                            <form method="POST" action="{{ route('admin.users.verify.approve', $user) }}">
                                ...
                                <button type="submit"
                                    ...
                                    @if ($user->verification_status === 'approved') disabled @endif>{{ __('messages.approve') }}</button>
                            </form>
                            <form method="POST" action="{{ route('admin.users.verify.reject', $user) }}"
                                onsubmit="return confirmRejectIdentity(this);">
                                @csrf
                                <input type="text" name="reason"
                                    placeholder="{{ __('messages.rejection_reason_required') }}"
                                    class="rounded-md border-gray-300 text-sm">
                                <button type="submit"
                                    class="inline-flex items-center px-3 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-md">{{ __('messages.reject') }}</button>
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <p><strong>{{ __('messages.role') }}:</strong>
                                @if ($user->role === 'admin')
                                    <span
                                        class="bg-red-100 text-red-800 px-2 py-1 rounded text-xs">{{ __('messages.admin') }}</span>
                                @elseif($user->role === 'seller')
                                    <span
                                        class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">{{ __('messages.seller') }}</span>
                                @else
                                    <span
                                        class="bg-gray-100 text-gray-800 px-2 py-1 rounded text-xs">{{ __('messages.buyer') }}</span>
                                @endif
                            </p>
                            <p><strong>{{ __('messages.wallet_balance_label') }}:</strong>
                                {{ currency($user->wallet_balance ?? 0) }}</p>
                            <p><strong>{{ __('messages.joined') }}:</strong> {{ $user->created_at->format('d M Y, H:i') }}
                            </p>
                            <p>
                                ...
                                @if ($user->suspended_at)
                                    ...
                                    <span class="block text-xs text-gray-500 mt-1">
                                        {{ __('messages.suspended_at_label') }}:
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <p><strong>{{ __('messages.public_notes_label') }}:</strong>
                                {{ $user->notes->where('is_public', true)->count() }}</p>
                            <p><strong>{{ __('messages.total_withdraws') }}:</strong> {{ $user->withdraws->count() }}</p>
                            <p><strong>{{ __('messages.pending_withdraws') }}:</strong>
                                {{ $user->withdraws->where('status', 'pending')->count() }}</p>
                            <p><strong>{{ __('messages.transactions_buyer') }}:</strong>
                                {{ $user->transactionsAsBuyer->count() }}</p>
                            <p><strong>{{ __('messages.transactions_seller') }}:</strong>
                                {{ $user->transactionsAsSeller->count() }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                @else
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <form method="POST" action="{{ route('admin.users.deactivate', $user) }}"
                            ...
                            <input type="text" name="reason"
                                placeholder="{{ __('messages.optional_reason_placeholder') }}"
                        ...
                        <form method="POST"
                            action="{{ $user->suspended_at ? route('admin.users.release', $user) : route('admin.users.suspend', $user) }}"
                            class="border rounded-lg p-4 shadow-sm space-y-3"
                            onsubmit="return confirmSuspend(this, {{ $user->suspended_at ? 'true' : 'false' }});">
                            @csrf
                            @if ($user->suspended_at)
                    ...
                    @if (!$user->isAccessible())
                        <form method="POST" action="{{ route('admin.users.activate', $user) }}" class="mt-4"
                                ...
                                @foreach ($user->withdraws->take(5) as $withdraw)
                                    <tr>
                                        <td class="px-4 py-3 text-sm text-gray-500">
                                            {{ $withdraw->created_at->format('d M Y') }}</td>
                            ...
                            <div class="border rounded-lg p-4 bg-gray-50">
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    <span class="text-sm font-semibold text-gray-800">
                                        {{ __('messages.admin_action_label') }}:
                                ...
                                <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-gray-600">
                                    <div class="bg-white border rounded p-3">
                                        <p class="font-semibold text-gray-700 mb-1">{{ __('messages.previous_status') }}
                                        </p>
                                        <p>{{ __('messages.user_status_active') }}:
                                            {{ $previous['is_active'] ?? false ? __('messages.yes') : __('messages.no') }}
                                        </p>
                                        <p>{{ __('messages.user_status_suspended') }}:
                                            {{ $previous['suspended_at'] ?? null ? __('messages.yes') : __('messages.no') }}
                                        </p>
                                    </div>
                                    <div class="bg-white border rounded p-3">
                                        <p class="font-semibold text-gray-700 mb-1">{{ __('messages.current_status') }}
                                        </p>
                                        <p>{{ __('messages.user_status_active') }}:
                                            {{ $current['is_active'] ?? false ? __('messages.yes') : __('messages.no') }}
                                        </p>
                                        <p>{{ __('messages.user_status_suspended') }}:
                                            {{ $current['suspended_at'] ?? null ? __('messages.yes') : __('messages.no') }}
                                        </p>
                                    </div>
                                </div>
                            </div>
