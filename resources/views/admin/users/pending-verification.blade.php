...
@section('title', 'User Verification Pending')
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Verifikasi User Pending</h2>
                <p class="text-sm text-gray-600 mt-1">Daftar user yang sudah upload dokumen identitas (KTP atau Kartu Pelajar) dan foto selfie serta memerlukan verifikasi identitas</p>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Dokumen Identitas & Selfie</th>
                                            ...
                                            @if($user->ktp_path)
                                                <div>
                                                    <span class="text-xs text-gray-600">
                                                        Jenis: {{ $user->document_type === 'kartu_pelajar' ? 'Kartu Pelajar' : 'KTP' }}
                                                    </span>
                                                    <a href="{{ route('admin.users.download-doc', ['user' => $user->id, 'type' => 'ktp']) }}" 
                                                       class="block text-blue-600 hover:text-blue-800 text-xs mt-1">
                                                        Download {{ $user->document_type === 'kartu_pelajar' ? 'Kartu Pelajar' : 'KTP' }}
                                                    </a>
                                                </div>
                                            @else
                                                <span class="text-gray-400 text-xs">Dokumen identitas tidak ada</span>
                                            @endif
                                            @if($user->selfie_path)
                                                <a href="{{ route('admin.users.download-doc', ['user' => $user->id, 'type' => 'selfie']) }}" 
                                                   class="text-blue-600 hover:text-blue-800 text-xs">Download Selfie</a>
                                            @else
                                                <span class="text-gray-400 text-xs">Selfie tidak ada</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            <form method="POST" action="{{ route('admin.users.verify.approve', $user) }}" class="inline">
                                                ...
                                                <button type="submit" 
                                                        ...
                                                        onclick="return confirm('Approve verifikasi untuk user ini?');">Approve</button>
                                            </form>
        ...
        @else
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 text-center">
                <p class="text-gray-600">Tidak ada user yang perlu verifikasi saat ini.</p>
            </div>
        @endif
    </div>
</div>
...
function confirmReject(form) {
    ...
    if (!reason || reason.trim() === '') {
        alert('Silakan isi alasan reject.');
        return false;
    }
    return confirm('Apakah Anda yakin ingin reject verifikasi user ini?');
}
</script>
@endpush
@endsection
