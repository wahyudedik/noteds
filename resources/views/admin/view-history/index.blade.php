        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    ...
                    <div class="text-2xl font-bold text-blue-600">{{ currency($stats['total_revenue'], 'IDR', 'IDR') }}
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="text-sm font-medium text-gray-500">{{ __('messages.today_revenue') }}</div>
                    <div class="text-2xl font-bold text-purple-600">{{ currency($stats['today_revenue'], 'IDR', 'IDR') }}
                    </div>
                    <div class="text-xs text-gray-500 mt-1">{{ number_format($stats['today_views']) }}
                        {{ __('messages.views_today') }}</div>
                </div>
            </div>
            ...
            <div class="bg-white shadow-sm rounded-lg p-4 mb-6">
                <form method="GET" action="{{ route('admin.view-history.index') }}" class="flex gap-4 flex-wrap">
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="{{ __('messages.search_by_note_user_ip') }}"
                    ...
                    <select name="valid" class="rounded-md border-gray-300 shadow-sm">
                        ...
                        <option value="1" {{ request('valid') === '1' ? 'selected' : '' }}>
                            {{ __('messages.valid_only') }}</option>
                        <option value="0" {{ request('valid') === '0' ? 'selected' : '' }}>
                            {{ __('messages.invalid_only') }}</option>
                    </select>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ $viewRevenue->note->title ?? 'N/A' }}</div>
                                            <div class="text-sm text-gray-500">{{ __('messages.owner') }}:
                                                {{ $viewRevenue->note->user->name ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if ($viewRevenue->user)
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('admin.view-history.show', $viewRevenue) }}"
                                                class="text-blue-600 hover:text-blue-900">{{ __('messages.view_details') }}</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
