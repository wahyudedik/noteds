...
@section('title', __('messages.view_history_title'))

@section('content')
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ __('messages.view_history_title') }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="text-sm font-medium text-gray-500">{{ __('messages.total_views') }}</div>
                <div class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_views']) }}</div>
            </div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="text-sm font-medium text-gray-500">{{ __('messages.valid_views') }}</div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="text-sm font-medium text-gray-500">{{ __('messages.today_revenue') }}</div>
                <div class="text-2xl font-bold text-purple-600">Rp {{ number_format($stats['today_revenue'], 2, ',', '.') }}</div>
                <div class="text-xs text-gray-500 mt-1">{{ number_format($stats['today_views']) }} {{ __('messages.views_today') }}</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div class="text-sm font-medium text-yellow-800">{{ __('messages.pending_validation') }}</div>
                <div class="text-xl font-bold text-yellow-900">{{ number_format($stats['pending_views']) }}</div>
            </div>
            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                <div class="text-sm font-medium text-red-800">{{ __('messages.rejected_views') }}</div>
        ...
        <div class="bg-white shadow-sm rounded-lg p-4 mb-6">
            <form method="GET" action="{{ route('admin.view-history.index') }}" class="flex gap-4 flex-wrap">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('messages.search_by_note_user_ip') }}"
                ...
                <select name="valid" class="rounded-md border-gray-300 shadow-sm">
                    ...
                    <option value="1" {{ request('valid') === '1' ? 'selected' : '' }}>{{ __('messages.valid_only') }}</option>
                    <option value="0" {{ request('valid') === '0' ? 'selected' : '' }}>{{ __('messages.invalid_only') }}</option>
                </select>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.ip_address') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.status') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.viewed_at') }}</th>
                            ...
                            @foreach($viewRevenues as $viewRevenue)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500">{{ __('messages.owner') }}: {{ $viewRevenue->note->user->name ?? 'N/A' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($viewRevenue->user)
                                        ...
                                        @else
                                            <span class="text-sm text-gray-500">{{ __('messages.guest') }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('admin.view-history.show', $viewRevenue) }}" class="text-blue-600 hover:text-blue-900">{{ __('messages.view_details') }}</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        @else
            <div class="bg-white shadow-sm rounded-lg p-8 text-center">
                <p class="text-gray-500">{{ __('messages.no_view_history_found') }}</p>
            </div>
        @endif
    </div>
</div>
