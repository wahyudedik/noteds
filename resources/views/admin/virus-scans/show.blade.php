            ...
            <div class="p-6">
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900">
                            <a href="{{ route('notes.show', $virusScan->note) }}" class="text-blue-600 hover:text-blue-800">
            ...
            <div class="p-6">
                <dl class="grid grid-cols-1 gap-4">
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-red-900 font-semibold">{{ $virusScan->threat_name ?? 'Unknown' }}</dd>
                    </div>
                    @if($virusScan->threat_details)
                    <div>
                        <dt class="text-sm font-medium text-gray-500">Threat Details</dt>
                        <dd class="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{{ $virusScan->threat_details }}</dd>
                    </div>
                    @endif
                    @if($virusScan->scan_result)
                    <div>
                        <dt class="text-sm font-medium text-gray-500">Scan Result</dt>
                        <dd class="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{{ $virusScan->scan_result }}</dd>
                    </div>
                    @endif
                </dl>
            </div>
            ...
            <div class="p-6">
                <dl class="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900">{{ $virusScan->quarantined_at->format('Y-m-d H:i:s') }}</dd>
                    </div>
                </dl>
            </div>
        </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <div class="p-6">
                <div class="flex gap-4">
                    @if($virusScan->isInfected() && !$virusScan->isQuarantined())
                        <form method="POST" action="{{ route('admin.virus-scans.quarantine', $virusScan) }}" class="inline">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white font-semibold rounded-md">
                                Quarantine File
                            </button>
                        </form>
                    @endif
                    ...
                    @if($virusScan->isQuarantined())
                        <form method="POST" action="{{ route('admin.virus-scans.restore', $virusScan) }}" class="inline">
                            @csrf
                            <div class="flex gap-2">
                                <input type="text" name="restore_path" value="{{ $virusScan->file_path }}" class="rounded-md border-gray-300 shadow-sm" required>
                    ...
                    @if($virusScan->isQuarantined())
                        <form method="POST" action="{{ route('admin.virus-scans.destroy', $virusScan) }}" class="inline" onsubmit="return confirm('Are you sure you want to permanently delete this quarantined file?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-md">
                                Delete Permanently
                            </button>
                        </form>
                    @endif
