...
<div class="py-8 sm:py-12">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form action="{{ route('admin.documentations.update', $documentation) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" class="hidden" required>{{ old('content', $documentation->content) }}</textarea>
                    @error('content')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" 
                            id="icon"
                            name="icon"
                            value="{{ old('icon', $documentation->icon) }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if($documentation->links && count($documentation->links) > 0)
        @foreach($documentation->links as $link)
            addLinkField('{{ addslashes($link['title'] ?? '') }}', '{{ addslashes($link['url'] ?? '') }}');
        @endforeach
    @endif
    
    // Load existing video URLs
    @if($documentation->video_urls && count($documentation->video_urls) > 0)
        @foreach($documentation->video_urls as $url)
            addVideoField('{{ addslashes($url) }}');
        @endforeach
    @endif
