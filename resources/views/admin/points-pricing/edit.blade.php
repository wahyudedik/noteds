...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="max-w-2xl mx-auto">
            <form action="{{ route('admin.points-pricing.update', $pointsPricingConfig) }}" method="POST"
                ...
                <div class="mb-6">
                    ...
                    <select name="type" id="type" required onchange="updateTypeFields()"
                        ...
                        <option value="discount"
                            {{ old('type', $pointsPricingConfig->type) === 'discount' ? 'selected' : '' }}>Discount (on
                            purchases)</option>
                        <option value="premium_feature"
                            {{ old('type', $pointsPricingConfig->type) === 'premium_feature' ? 'selected' : '' }}>Premium
                            Feature (days)</option>
                    </select>
                    @error('type')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="points_required" id="points_required"
                        value="{{ old('points_required', $pointsPricingConfig->points_required) }}" min="1" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                ...
                <div class="mb-6">
                    ...
                    <input type="datetime-local" name="expires_at" id="expires_at"
                        value="{{ old('expires_at', $pointsPricingConfig->expires_at ? $pointsPricingConfig->expires_at->format('Y-m-d\TH:i') : '') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1"
                            {{ old('is_active', $pointsPricingConfig->is_active) ? 'checked' : '' }}
