                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">
                            {{ $landingPage->title ?: __('messages.landing_page_section') }}</h1>
                        <p class="mt-2 text-base text-gray-600">{{ __('messages.view_section_details') }}</p>
                    </div>
                    <div class="flex gap-3">
                        <a href="{{ route('admin.landing-page.edit', $landingPage) }}"
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.section_type') }}</label>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.status') }}</label>
                                <p class="mt-2">
                        ...
                        <div>
                            <label
                                class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.title') }}</label>
                            <p class="mt-2 text-sm text-gray-900">{{ $landingPage->title ?: '—' }}</p>
                        </div>

                        <div>
                            <label
                                class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.subtitle_description') }}</label>
                        ...
                        <div>
                            <label
                                class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">{{ __('messages.content') }}</label>
                        ...
                        <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                            <div>
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.display_order') }}</label>
                                <p class="mt-2 text-sm font-medium text-gray-900">{{ $landingPage->order ?? '0' }}</p>
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.alignment') }}</label>
                        ...
                        @if ($landingPage->background_color || $landingPage->text_color)
                            <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                                @if ($landingPage->background_color)
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.background_color') }}</label>
                                        <div class="mt-2 flex items-center gap-2">
                                            <div class="w-8 h-8 rounded border border-gray-300"
                                                style="background-color: {{ $landingPage->background_color }}"></div>
                                            <p class="text-sm font-mono text-gray-900">{{ $landingPage->background_color }}
                                            </p>
                                        </div>
                                    </div>
                                @endif
                                @if ($landingPage->text_color)
                                    <div>
                                        <label
                                            class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.text_color') }}</label>
                                        <div class="mt-2 flex items-center gap-2">
                                            <div class="w-8 h-8 rounded border border-gray-300"
                                                style="background-color: {{ $landingPage->text_color }}"></div>
                        ...
                        @if ($landingPage->image_url)
                            <div class="pt-4 border-t border-gray-200">
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">{{ __('messages.image') }}</label>
                                <img src="{{ $landingPage->image_url }}" alt="{{ $landingPage->title }}"
                        ...
                        @if ($landingPage->section_type === 'promo' && ($landingPage->valid_from || $landingPage->valid_until))
                            <div class="pt-4 border-t border-gray-200">
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.valid_period') }}</label>
                                <div class="mt-2 grid grid-cols-2 gap-4">
                                    @if ($landingPage->valid_from)
                                        <div>
                                            <p class="text-xs text-gray-600">{{ __('messages.from') }}</p>
                                            <p class="text-sm font-medium text-gray-900">
                                                {{ localized_time($landingPage->valid_from, 'date') }}</p>
                                        </div>
                                    @endif
                                    @if ($landingPage->valid_until)
                                        <div>
                                            ...
                                            <p class="text-sm font-medium text-gray-900">
                                                {{ localized_time($landingPage->valid_until, 'date') }}</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                        ...
                        <div class="space-y-3">
                            <div>
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.created_by') }}</label>
                                <div class="mt-2 flex items-center gap-2">
                                    <img src="{{ $landingPage->creator->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($landingPage->creator->name) }}"
                                        alt="{{ $landingPage->creator->name }}" class="w-8 h-8 rounded-full">
                                    <p class="text-sm font-medium text-gray-900">{{ $landingPage->creator->name }}</p>
                                </div>
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.created_at') }}</label>
                                <p class="text-sm text-gray-900">{{ localized_time($landingPage->created_at) }}</p>
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.last_updated') }}</label>
                    ...
                    <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                        ...
                        <div class="space-y-2">
                            <a href="{{ route('admin.landing-page.edit', $landingPage) }}"
                                class="w-full inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700 transition-all duration-200">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            ...
                            <form action="{{ route('admin.landing-page.destroy', $landingPage) }}" method="POST"
                                class="w-full delete-section-form">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full inline-flex items-center justify-center px-4 py-2 border border-red-300 text-sm font-medium rounded-lg text-red-700 bg-white hover:bg-red-50 transition-all duration-200">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
