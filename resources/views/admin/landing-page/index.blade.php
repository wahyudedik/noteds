...
@section('title', __('messages.landing_page_sections'))
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">{{ __('messages.landing_page_sections') }}</h1>
                        <p class="mt-2 text-base text-gray-600">{{ __('messages.manage_homepage_sections') }}</p>
                    </div>
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('admin.landing-page.create') }}"
            ...
            <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6 mb-6">
                <form method="GET" action="{{ route('admin.landing-page.index') }}" class="flex gap-4">
                    <select name="type"
                        class="rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500">
                    ...
                    <button type="submit"
                        class="px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700">
                        {{ __('messages.filter') }}
                    </button>
                    @if (request('type'))
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('messages.order') }}</th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('messages.status') }}</th>
                                    <th
                                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        {{ __('messages.valid_period') }}</th>
                                    <th
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ $section->title ?: __('messages.no_title') }}</div>
                                            @if ($section->subtitle)
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            @if ($section->section_type === 'promo')
                                                @if ($section->valid_from && $section->valid_until)
                                                    {{ localized_time($section->valid_from, 'date') }} -
                                                    {{ localized_time($section->valid_until, 'date') }}
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('admin.landing-page.show', $section) }}"
                                                ...
                                                <a href="{{ route('admin.landing-page.edit', $section) }}"
                                                ...
                                                <form action="{{ route('admin.landing-page.destroy', $section) }}"
                                                    method="POST" class="inline delete-section-form">
                                                    @csrf
                                                    @method('DELETE')
            ...
            @else
                <div class="bg-white shadow-sm rounded-lg border border-gray-200 text-center py-16 px-6">
                    ...
                    <h3 class="mt-4 text-lg font-medium text-gray-900">{{ __('messages.no_sections_found') }}</h3>
                    <p class="mt-2 text-sm text-gray-500">{{ __('messages.get_started_creating_first_section') }}</p>
                    <div class="mt-6">
                        <a href="{{ route('admin.landing-page.create') }}"
                ...
                document.querySelectorAll('.delete-section-form').forEach(form => {
                    form.addEventListener('submit', function(e) {
                        ...
                        if (typeof Swal !== 'undefined') {
                            ...
                            if (confirm(@json(__('messages.delete_section_confirm')))) {
                                formElement.submit();
                            }
                        }
                    });
