                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.valid_period') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($sections as $section)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $section->title ?: __('messages.no_title') }}</div>
                                        @if($section->subtitle)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        @if($section->section_type === 'promo')
                                            ...
                                            @elseif($section->valid_from)
                                                {{ __('messages.from') }} {{ localized_time($section->valid_from, 'date') }}
                                            @elseif($section->valid_until)
                                                {{ __('messages.until') }} {{ localized_time($section->valid_until, 'date') }}
                                            @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('admin.landing-page.show', $section) }}" class="text-blue-600 hover:text-blue-900" title="{{ __('messages.view') }}">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('admin.landing-page.edit', $section) }}" class="text-green-600 hover:text-green-900" title="{{ __('messages.edit') }}">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
