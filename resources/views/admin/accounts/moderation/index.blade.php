...
@section('title', __('messages.account_moderation'))
...
<div class="py-12 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">{{ __('messages.account_moderation') }}</h1>
        ...
        <div class="bg-white shadow-sm rounded-lg p-6 mb-8">
            <form method="GET" action="{{ route('admin.accounts.moderation.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if($search || $reportStatus)
                        <a href="{{ route('admin.accounts.moderation.index') }}" class="inline-flex items-center justify-center px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-medium rounded-lg">
                            Clear
                        </a>
                    @endif
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($users as $moderatedUser)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $moderatedUser->name }}</div>
                                    <div class="text-sm text-gray-500">{{ '@' . $moderatedUser->username }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">
                                        {{ ucfirst($moderatedUser->role) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $moderatedUser->pending_reports_count > 0 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                                            {{ $moderatedUser->pending_reports_count }} pending
                                        </span>
                                        <span class="text-xs text-gray-500">Total: {{ $moderatedUser->account_reports_count }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $moderatedUser->created_at->format('d M Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('admin.accounts.moderation.show', $moderatedUser) }}" class="inline-flex items-center px-3 py-1.5 text-sm text-blue-600 hover:text-blue-800">
                                        View
                                    </a>
                                </td>
                            </tr>
