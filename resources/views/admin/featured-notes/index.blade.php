    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-yellow-200 mb-6">
            <div class="px-6 py-4 border-b border-yellow-100 bg-yellow-50 flex items-center justify-between">
                ...
                <a href="{{ route('admin.settings.index') }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium">{{ __('featured.admin.pricing_panel_link') }}</a>
            </div>
            <div class="p-6">
                        ...
                        @foreach ($featuredLocationLabels as $location => $label)
                            <div class="border border-gray-200 rounded-lg p-4">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <span class="text-xs text-gray-500">{{ __('featured.admin.location_key_label', ['key' => $location]) }}</span>
                                </div>
        ...
        <div class="bg-white shadow-sm rounded-lg p-4 mb-6">
            <form method="GET" action="{{ route('admin.featured-notes.index') }}" class="flex gap-4">
                <select name="status" class="rounded-md border-gray-300 shadow-sm">
                    ...
                    <option value="pending" {{ request('status') === 'pending' ? 'selected' : '' }}>{{ __('featured.status_pending') }}</option>
                    <option value="active" {{ request('status') === 'active' ? 'selected' : '' }}>{{ __('featured.status_active') }}</option>
                    <option value="expired" {{ request('status') === 'expired' ? 'selected' : '' }}>{{ __('featured.status_expired') }}</option>
                    <option value="cancelled" {{ request('status') === 'cancelled' ? 'selected' : '' }}>{{ __('featured.status_cancelled') }}</option>
                </select>
                <select name="location" class="rounded-md border-gray-300 shadow-sm">
                    <option value="">{{ __('featured.admin.filter.location_all') }}</option>
                    <option value="marketplace_grid" {{ request('location') === 'marketplace_grid' ? 'selected' : '' }}>{{ __('featured.locations.marketplace_grid') }}</option>
                    <option value="marketplace_banner" {{ request('location') === 'marketplace_banner' ? 'selected' : '' }}>{{ __('featured.locations.marketplace_banner') }}</option>
                    <option value="landing_hero" {{ request('location') === 'landing_hero' ? 'selected' : '' }}>{{ __('featured.locations.landing_hero') }}</option>
                    <option value="landing_carousel" {{ request('location') === 'landing_carousel' ? 'selected' : '' }}>{{ __('featured.locations.landing_carousel') }}</option>
                </select>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('featured.admin.table.note') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('featured.admin.table.seller') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('featured.admin.table.location') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('featured.admin.table.duration') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('featured.admin.table.price') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('featured.admin.table.status') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('featured.admin.table.dates') }}</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('featured.admin.table.action') }}</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        @if($featured->start_date && $featured->end_date)
                                            <span class="text-sm text-gray-900">{{ __('featured.admin.date_range', [
                                                'start' => $featured->start_date->format('d M Y'),
                                                'end' => $featured->end_date->format('d M Y')
                                            ]) }}</span>
                                        @else
                                            <span class="text-gray-400">{{ __('featured.admin.dates_not_set') }}</span>
                                        @endif
                                    </td>
