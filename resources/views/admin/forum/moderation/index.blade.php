...
<div class="py-12 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-sm rounded-lg p-6 mb-8">
            <form method="GET" action="{{ route('admin.forum.moderation.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Search</label>
                    <input type="text" name="search" value="{{ $search }}" placeholder="Search content or user..."
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if($search || $status || $reportStatus)
                        <a href="{{ route('admin.forum.moderation.index') }}" class="inline-flex items-center justify-center px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-medium rounded-lg">
                            Clear
                        </a>
                    @endif
                </div>
                        ...
                        @forelse($posts as $post)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-sm text-gray-900 max-w-md">
                                    <p class="font-medium text-gray-900">{{ Str::limit($post->content, 120) }}</p>
                                    <div class="mt-2 flex items-center gap-2">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $post->parent_id ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                                            {{ $post->parent_id ? 'Reply' : 'Top Level' }}
                                        </span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $post->pending_reports_count > 0 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                                            {{ $post->pending_reports_count }} pending
                                        </span>
                                        <span class="text-xs text-gray-500">Total: {{ $post->reports_count }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($post->is_hidden)
                                        ...
                                        @if($post->hidden_at)
                                            <div class="text-xs text-gray-500 mt-1">since {{ optional($post->hidden_at)->format('d M Y H:i') }}</div>
                                        @endif
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-y-2">
                                    <a href="{{ route('admin.forum.moderation.show', $post) }}" class="inline-flex items-center px-3 py-1.5 text-sm text-blue-600 hover:text-blue-800">
                                        View
                                    </a>
