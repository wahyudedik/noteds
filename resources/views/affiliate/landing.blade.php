...
@section('content')
    ...
    <div
        class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 relative overflow-hidden">
        <!-- Animated Background Elements -->
        <div
            class="absolute top-20 left-10 w-72 h-72 bg-blue-300 dark:bg-blue-900 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float">
        </div>
        <div class="absolute top-40 right-10 w-72 h-72 bg-purple-300 dark:bg-purple-900 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float"
            style="animation-delay: 2s;"></div>
        <div class="absolute -bottom-8 left-20 w-72 h-72 bg-pink-300 dark:bg-pink-900 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float"
            ...
            @if ($link->landing_page_content)
                ...
                <div class="max-w-4xl mx-auto animate-slide-down">
                    ...
                    <div class="text-center mb-8">
                        <span
                            class="inline-block px-4 py-2 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 text-white text-sm font-bold uppercase tracking-wide">
                    ...
                    <div class="mt-12 flex justify-center gap-8 flex-wrap">
                        <div class="text-center">
                            ...
                            <p class="text-sm text-slate-600 dark:text-slate-400 font-medium">
                                {{ __('messages.secure') ?? 'Secure' }}</p>
                        </div>
                        <div class="text-center">
                            <div class="text-2xl mb-2">⚡</div>
                            <p class="text-sm text-slate-600 dark:text-slate-400 font-medium">
                                {{ __('messages.instant') ?? 'Instant Access' }}</p>
                        </div>
                        <div class="text-center">
                            <div class="text-2xl mb-2">✓</div>
                            <p class="text-sm text-slate-600 dark:text-slate-400 font-medium">
                                {{ __('messages.verified') ?? '100% Verified' }}</p>
                        </div>
                    </div>
                </div>
            @else
                ...
                <div class="max-w-4xl mx-auto animate-slide-down">
                    ...
                    <div class="grid md:grid-cols-3 gap-6 mb-12 animate-slide-up">
                        @foreach ([['icon' => '📚', 'title' => 'Rich Content', 'desc' => 'Curated premium notes and templates'], ['icon' => '💰', 'title' => 'Best Prices', 'desc' => 'Exclusive affiliate discounts'], ['icon' => '⚡', 'title' => 'Instant Access', 'desc' => 'Start using immediately']] as $feature)
                            <div class="feature-card glass-effect rounded-xl p-6 text-center">
                                <div class="text-4xl mb-4">{{ $feature['icon'] }}</div>
                                <h3 class="font-bold text-lg text-slate-900 dark:text-white mb-2">{{ $feature['title'] }}
                                </h3>
                    ...
                    <div class="grid md:grid-cols-3 gap-6 animate-slide-up" style="animation-delay: 0.4s;">
                        @foreach ([['num' => '10K+', 'label' => 'Active Users'], ['num' => '50K+', 'label' => 'Premium Notes'], ['num' => '4.9★', 'label' => 'Rating']] as $stat)
