    ...
    <div class="mb-6" x-data="window.mediaGallery()" x-init="init()">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            @foreach($images as $index => $image)
                @php
                    $imageUrl = is_string($image) ? asset('storage/' . $image) : (is_array($image) ? ($image['url'] ?? asset('storage/' . ($image['path'] ?? ''))) : $image);
                    $imageAlt = is_array($image) ? ($image['alt'] ?? "Image " . ($index + 1)) : "Image " . ($index + 1);
                @endphp
                <div 
                    class="relative group cursor-pointer overflow-hidden rounded-lg border-2 border-gray-200 hover:border-blue-400 transition-all duration-200 aspect-square"
        ...
        >
            ...
            <button 
                x-show="currentIndex < imageCount - 1"
                @click="nextImage()"
            ...
            <!-- Zoom Controls -->
            <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-10 flex items-center gap-2 bg-black bg-opacity-50 rounded-full px-4 py-2">
                <button 
                    @click="zoomOut()"
                ...
                <button 
                    @click="resetZoom()"
                    x-show="zoomLevel > 1 || translateX !== 0 || translateY !== 0"
