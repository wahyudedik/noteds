    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="flex space-x-8 overflow-x-auto" role="tablist">
            @foreach ($tabs as $key => $tab)
                <button onclick="switchTab('{{ $key }}')" role="tab"
                    aria-selected="{{ $activeTab === $key ? 'true' : 'false' }}" aria-controls="panel-{{ $key }}"
...
<script>
    function switchTab(tabKey) {
        ...
        @foreach ($tabs as $key => $tab)
            ...
            document.querySelector('[aria-controls="panel-{{ $key }}"]').setAttribute('aria-selected',
            'false');
            document.querySelector('[aria-controls="panel-{{ $key }}"]').classList.remove('border-blue-500',
                'text-blue-600');
            document.querySelector('[aria-controls="panel-{{ $key }}"]').classList.add('border-transparent',
                'text-gray-600');
        @endforeach
        ...
        document.querySelector('[aria-controls="panel-' + tabKey + '"]').classList.add('border-blue-500',
            'text-blue-600');
        document.querySelector('[aria-controls="panel-' + tabKey + '"]').classList.remove('border-transparent',
            'text-gray-600');
