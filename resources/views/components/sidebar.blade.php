    ...
    <aside 
        ...
        <div class="flex items-center justify-between h-16 px-4 border-b border-gray-200 dark:border-gray-700">
            <a href="/" class="flex items-center gap-2 min-w-0">
                ...
                <span x-show="isExpanded" x-transition class="text-xl font-bold text-gray-900 dark:text-white whitespace-nowrap overflow-hidden">
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-2">
            @foreach ($menuGroups as $group)
                @if ($group['title'])
                    <div x-show="isExpanded" x-transition class="px-3 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                        {{ $group['title'] }}
                    </div>
                @endif
                <div class="space-y-1">
                    @foreach ($group['items'] as $item)
                        <a 
                            ...
                            title="{{ $item['label'] }}">
                            <span class="flex-shrink-0 {!! $item['active'] ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 group-hover:text-gray-700 dark:group-hover:text-gray-300' !!}">
                                {!! $item['icon'] !!}
                            </span>
            ...
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center flex-shrink-0">
                    @if (auth()->user()->avatar)
                        ...
                        @endif
                    @endif
                    <span class="text-sm font-semibold text-white" style="display: {{ auth()->user()->avatar ? 'none' : 'flex' }};">
