@php
    ...
    if ($user) {
        if ($user->role === 'user_workspaces') {
            ...
            // Leaderboards (only for seller and buyer, not admin)
            if (!$isAdmin) {
                $mainItems[] = [
                    'label' => 'Leaderboards',
                ...
                if ($isBuyer) {
                    $studioItems[] = [
                        ...
                        'icon' =>
                            '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>',
                        'active' =>
                            request()->routeIs('studio.orders.index') ||
                            (request()->routeIs('studio.orders.show') ||
                                (request()->routeIs('studio.orders.*') &&
                                    !request()->routeIs('studio.orders.pending-approvals'))),
                    ];

                    $studioItems[] = [
                        'label' => 'Pending Approvals',
                        'href' => route('studio.orders.pending-approvals'), // View submitted work awaiting approval
                        'icon' =>
                            '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',
                        'active' => request()->routeIs('studio.orders.pending-approvals'),
                    ];
                }
            ...
            // Seller Tools - Only for sellers, NOT admin
            if ($isSeller) {
                $menuGroups[] = [
            ...
            // Buyer Library - Only for Buyers, NOT admin or seller
            if ($isBuyer) {
                $menuGroups[] = [
