    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <p class="text-gray-600 dark:text-gray-400">Order #{{ $dispute->service_order_id }} -
                            {{ $dispute->serviceOrder->title }}</p>
                    </div>
                    <span
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-900 dark:text-white">
                            {{ $dispute->created_at->format('M d, Y H:i') }}</p>
                    </div>
                    <div>
            ...
            @if ($dispute->evidence->count() > 0)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Evidence
                        ({{ $dispute->evidence->count() }})</h2>
                    <div class="space-y-3">
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                                        Submitted by {{ $file->submittedBy->name }} on
                                        {{ $file->created_at->format('M d, Y H:i') }}
                                    </p>
                                    @if ($file->description)
                                        <p class="text-sm text-gray-700 dark:text-gray-300 mt-1">{{ $file->description }}
                                        </p>
                                    @endif
                                </div>
                    ...
                    @if ($dispute->isOpen())
                        <form action="{{ route('studio.disputes.add-evidence', $dispute) }}" method="POST"
                            enctype="multipart/form-data" class="mt-6 pt-6 border-t border-gray-300 dark:border-gray-600">
                            @csrf
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <textarea name="description" id="description" rows="3" placeholder="Describe this evidence..."
                                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"></textarea>
                                </div>
            ...
            @if ($dispute->isResolved())
                <div
                    class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg p-6 mb-6">
                    ...
                    <div class="space-y-3 text-sm">
                        <div>
                            ...
                            <p class="font-semibold text-green-900 dark:text-green-200">
                                {{ $dispute->getResolutionTypeLabel() }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-green-900 dark:text-green-200">
                                {{ $dispute->resolved_at->format('M d, Y H:i') }}</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <!-- Back Button -->
            <div>
                <a href="{{ route('studio.orders.show', $dispute->serviceOrder) }}"
