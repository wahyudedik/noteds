    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('studio.messages.compose') }}"
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <img src="{{ $conversation['recipient']->profile_photo_url }}"
                                                alt="{{ $conversation['recipient']->name }}" class="w-10 h-10 rounded-full">
                                            <div>
                                                <p class="font-medium text-gray-900 dark:text-white">
                                                    {{ $conversation['recipient']->name }}</p>
                                                <p class="text-sm text-gray-500 dark:text-gray-400">@{{ $conversation['recipient'] - > username }}
                                                </p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">
                                        <p class="truncate max-w-xs">
                                            {{ Str::limit($conversation['lastMessage']->message, 50) }}</p>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                        {{ $conversation['lastMessage']->created_at->format('M d, Y H:i') }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <a href="{{ route('studio.messages.show', $conversation['recipient']) }}"
