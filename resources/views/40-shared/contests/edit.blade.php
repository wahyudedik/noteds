    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h2 class="text-2xl font-bold text-gray-900 mb-6">Edit Contest</h2>

                @if ($contest->status !== 'draft')
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                        <p class="text-yellow-800"><strong>Note:</strong> You can only edit draft contests. This contest is
                            currently {{ $contest->status }}.</p>
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('contests.update', $contest) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $contest->title) }}"
                            required
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4" required
                            ...
                            placeholder="Describe your contest...">{{ old('description', $contest->description) }}</textarea>
                        @error('description')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="type" name="type" required
                                ...
                                <option value="monthly" {{ old('type', $contest->type) === 'monthly' ? 'selected' : '' }}>
                                    Monthly</option>
                                <option value="themed" {{ old('type', $contest->type) === 'themed' ? 'selected' : '' }}>
                                    Themed</option>
                                <option value="custom" {{ old('type', $contest->type) === 'custom' ? 'selected' : '' }}>
                                    Custom</option>
                            </select>
                            @error('type')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="text" id="theme" name="theme" value="{{ old('theme', $contest->theme) }}"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="draft" {{ old('status', $contest->status) === 'draft' ? 'selected' : '' }}>
                                    Draft</option>
                                <option value="open" {{ old('status', $contest->status) === 'open' ? 'selected' : '' }}>
                                    Open</option>
                                <option value="voting"
                                    {{ old('status', $contest->status) === 'voting' ? 'selected' : '' }}>Voting</option>
                                <option value="closed"
                                    {{ old('status', $contest->status) === 'closed' ? 'selected' : '' }}>Closed</option>
                            </select>
                            @error('status')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" id="max_entries_per_user" name="max_entries_per_user"
                                value="{{ old('max_entries_per_user', $contest->max_entries_per_user) }}" min="1"
                                max="20" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" id="voting_start_date" name="voting_start_date"
                                value="{{ old('voting_start_date', $contest->voting_start_date?->format('Y-m-d')) }}"
                                class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" id="voting_end_date" name="voting_end_date"
                                value="{{ old('voting_end_date', $contest->voting_end_date?->format('Y-m-d')) }}"
                                class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="prizes_json" name="prizes_json" rows="4"
                            ...
                            placeholder='["Rp 500.000", "Rp 300.000", "Rp 200.000"]'>{{ old('prizes_json', json_encode($contest->prizes ?? [])) }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="url" id="banner_image" name="banner_image"
                            value="{{ old('banner_image', $contest->banner_image) }}"
                            class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
