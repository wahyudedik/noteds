                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label for="summary" class="block text-sm font-medium text-gray-700 mb-2">
                            {{ __('messages.summary') }} <span class="text-xs text-gray-500">{{ __('messages.summary_optional_ai') }}</span>
                        </label>
                    ...
                    <div id="preview-content-wrapper" style="display: {{ old('price', 0) > 0 ? 'block' : 'none' }};">
                        ...
                        <p class="mt-1 text-xs text-gray-500">
                            ...
                            @if(empty(old('preview_content'))) {{ __('messages.auto_generated_from_content') }} @endif
                        </p>
                    ...
                    <div>
                        ...
                        @if(!auth()->user()->hasPremium())
                            <p class="mt-2 text-xs text-yellow-600 flex items-center">
                                ...
                                {{ __('messages.file_size_limit_basic') }} <a href="{{ route('subscription.create') }}" class="font-semibold hover:underline">{{ __('messages.upgrade_to_premium') }}</a> {{ __('messages.file_size_limit_premium') }}
                            </p>
                        @endif
                    </div>
                        ...
                        <div>
                            ...
                            <select name="folder_id" id="folder_id"
                                ...
                                @foreach($folders as $folder)
                                    <option value="{{ $folder->id }}" {{ old('folder_id', $selectedFolder?->id) == $folder->id ? 'selected' : '' }}>
                                        {{ $folder->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="workspace_id" id="workspace_id"
                                ...
                                @foreach($workspaces as $workspace)
                                    <option value="{{ $workspace->id }}" {{ old('workspace_id', $selectedWorkspace?->id) == $workspace->id ? 'selected' : '' }}>
