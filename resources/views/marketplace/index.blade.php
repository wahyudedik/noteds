            ...
            <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6 mb-8" x-data="{ 
                ...
                <form method="GET" action="{{ route('marketplace.index') }}" class="space-y-4" id="filterForm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                ...
                                <div class="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1">
                                    ...
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false" x-cloak
                                            ...
                                            <div class="p-2">
                                                <div class="text-xs font-semibold text-gray-700 mb-2 px-2">Recent Searches</div>
                                                ...
                                                <a href="{{ route('marketplace.index', array_merge($history->filters ?? [], ['search' => $history->query])) }}"
                                                    ...
                                                    <div class="font-medium truncate">{{ $history->query ?: 'Filters only' }}</div>
                                                    <div class="text-xs text-gray-500">{{ $history->result_count }} results • {{ $history->searched_at->diffForHumans() }}</div>
                                                </a>
                                                @endforeach
                                            </div>
                                        </div>
                                    ...
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false" x-cloak
                                            class="absolute right-0 mt-2 w-64 bg-white border border-gray-300 rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
                                            <div class="p-2">
                                                <div class="text-xs font-semibold text-gray-700 mb-2 px-2">Saved Searches</div>
                                                @foreach($savedSearches as $saved)
                                                <div class="flex items-center justify-between px-2 py-1.5 hover:bg-gray-50 rounded group">
                                                    <a href="{{ route('marketplace.index', array_merge($saved->filters ?? [], ['search' => $saved->query])) }}"
                                                        class="flex-1 text-sm text-gray-700">
                                                        <div class="font-medium truncate">{{ $saved->name }}</div>
                                                        <div class="text-xs text-gray-500 truncate">{{ $saved->query ?: 'Filters only' }}</div>
                                                    </a>
                                                    <button type="button" onclick="deleteSavedSearch('{{ $saved->id }}')"
                        ...
                        <div>
                            <label for="tags"
                                class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.tag') }} (Multi-select)</label>
                            <div class="relative" x-data="{ open: false }">
                                <button type="button" @click="open = !open"
                                    ...
                                    <span class="text-sm text-gray-700" x-text="selectedTags.length > 0 ? selectedTags.length + ' tags selected' : 'All tags'"></span>
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div x-show="open" @click.away="open = false" x-cloak
                                    ...
                                    <div class="p-2 space-y-1">
                                        <input type="text" placeholder="Search tags..." @input="filterTags = $event.target.value.toLowerCase()"
                                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                        <template x-for="tag in filteredTags" :key="tag.id">
                                            <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                                <input type="checkbox" :value="tag.id" x-model="selectedTags"
                                                    :name="'tags[]'" class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                                <span class="text-sm text-gray-700" x-text="tag.name + ' (' + tag.count + ')'"></span>
                                            </label>
                                        </template>
                                    </div>
                                </div>
                        ...
                        <div>
                            ...
                            <select name="ecosystem" id="ecosystem"
                                ...
                                @foreach($ecosystems as $value => $label)
                                    <option value="{{ $value }}" {{ request('ecosystem') === $value ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            <label for="languages" class="block text-sm font-medium text-gray-700 mb-2">Bahasa (Multi-select)</label>
                            <div class="relative" x-data="{ open: false }">
                                <button type="button" @click="open = !open"
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-200 bg-white text-left px-3 py-2 flex items-center justify-between">
                                    <span class="text-sm text-gray-700" x-text="selectedLanguages.length > 0 ? selectedLanguages.length + ' languages' : 'All languages'"></span>
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                    </svg>
                                </button>
                                <div x-show="open" @click.away="open = false" x-cloak
                                    ...
                                    <div class="p-2 space-y-1">
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="en" x-model="selectedLanguages" name="languages[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                            <span class="text-sm text-gray-700">English</span>
                                        </label>
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="id" x-model="selectedLanguages" name="languages[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                            <span class="text-sm text-gray-700">Bahasa Indonesia</span>
                                        </label>
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="ar" x-model="selectedLanguages" name="languages[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                ...
                                <input type="hidden" name="language" x-bind:value="selectedLanguages.length === 1 ? selectedLanguages[0] : ''">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <!-- Author Search -->
                        <div>
                            <label for="author" class="block text-sm font-medium text-gray-700 mb-2">Search by Author</label>
                        ...
                        <div class="col-span-full">
                            ...
                            <div class="flex items-center gap-4">
                                <input type="number" name="min_price" x-model="minPrice" @input="priceRange[0] = parseInt($event.target.value) || 0"
                                ...
                                <span class="text-sm text-gray-600" x-text="'Rp ' + (minPrice || 0).toLocaleString('id-ID')"></span>
                            </div>
                            <div class="flex items-center gap-4 mt-2">
                                <input type="number" name="max_price" x-model="maxPrice" @input="priceRange[1] = parseInt($event.target.value) || 999999"
                                    placeholder="Max" min="0" step="0.01"
                                    class="w-32 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500">
                                <input type="range" x-model="priceRange[1]" @input="maxPrice = $event.target.value"
                                    min="0" max="1000000" step="1000" class="flex-1">
                                <span class="text-sm text-gray-600" x-text="'Rp ' + (maxPrice || 999999).toLocaleString('id-ID')"></span>
                            </div>
                        </div>
                        ...
                        <div>
                            <label for="seller_verified" class="block text-sm font-medium text-gray-700 mb-2">Seller Status</label>
                            <select name="seller_verified" id="seller_verified"
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-200">
                                <option value="">All sellers</option>
                                <option value="1" {{ request('seller_verified') == '1' ? 'selected' : '' }}>Verified sellers only</option>
                            </select>
                        </div>

                        <!-- Seller Type Filter -->
                        <div>
                            <label for="seller_type" class="block text-sm font-medium text-gray-700 mb-2">Seller Type</label>
                            <select name="seller_type" id="seller_type"
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-200">
                                <option value="">All sellers</option>
                                <option value="top_rated" {{ request('seller_type') == 'top_rated' ? 'selected' : '' }}>Top-rated sellers (4.5+ stars)</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative" x-data="{ open: false }">
                                <button type="button" @click="open = !open"
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-200 bg-white text-left px-3 py-2 flex items-center justify-between">
                                    <span class="text-sm text-gray-700" x-text="selectedFileTypes.length > 0 ? selectedFileTypes.length + ' types' : 'All file types'"></span>
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                    </svg>
                                </button>
                                <div x-show="open" @click.away="open = false" x-cloak
                                    ...
                                    <div class="p-2 space-y-1">
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="pdf" x-model="selectedFileTypes" name="file_type[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                            <span class="text-sm text-gray-700">PDF</span>
                                        </label>
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="doc" x-model="selectedFileTypes" name="file_type[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                            <span class="text-sm text-gray-700">DOC / DOCX</span>
                                        </label>
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="zip" x-model="selectedFileTypes" name="file_type[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                            <span class="text-sm text-gray-700">ZIP / RAR</span>
                                        </label>
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="image" x-model="selectedFileTypes" name="file_type[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                            <span class="text-sm text-gray-700">Images</span>
                                        </label>
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="video" x-model="selectedFileTypes" name="file_type[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                            <span class="text-sm text-gray-700">Videos</span>
                                        </label>
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="audio" x-model="selectedFileTypes" name="file_type[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                            <span class="text-sm text-gray-700">Audio</span>
                                        </label>
                                        <label class="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                            <input type="checkbox" value="code" x-model="selectedFileTypes" name="file_type[]"
                                                class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <select name="sort" id="sort"
                                ...
                                <option value="trending" {{ request('sort') == 'trending' ? 'selected' : '' }}>🔥 Trending (7
                    ...
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if(request()->hasAny(['search', 'tag', 'tags', 'min_price', 'max_price', 'sort', 'ecosystem', 'language', 'languages', 'min_rating', 'seller_verified', 'seller_type', 'file_type', 'author', 'date_from', 'date_to']))
                            ...
                            <div
                                ...
                                @if($note->hasThumbnails())
                                    <div class="h-48 overflow-hidden bg-gray-100">
                                        <img src="{{ Storage::url($note->thumbnails[0]) }}" alt="{{ $note->title }}" loading="lazy"
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="mb-4">
                                        <h3
                                            ...
                                            <a href="{{ route('marketplace.show', $note) }}" class="featured-click-tracking"
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex flex-wrap items-center gap-2 mb-4">
                                    ...
                                    @if($note->price > 0)
                                        ...
                                        @else
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-800 font-semibold">
                                                {{ currency($note->price) }}
                                            </span>
                                        @endif
                                    @else
                                        ...
                                        <div
                                            ...
                                            @if($note->user->avatar)
                                                @if(str_starts_with($note->user->avatar, 'http'))
                                                    <img src="{{ $note->user->avatar }}" alt="{{ $note->user->name }}" loading="lazy"
                                                        class="w-6 h-6 rounded-full object-cover">
                                                @else
                                                    <img src="{{ Storage::url($note->user->avatar) }}" alt="{{ $note->user->name }}"
