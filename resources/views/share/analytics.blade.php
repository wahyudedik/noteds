...
@section('title', 'Share Analytics - Earn Commission')
...
<div class="py-8 sm:py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            @if($shareReferrals->count() > 0)
                <div class="divide-y divide-gray-200">
                    @foreach($shareReferrals as $shareReferral)
                        <div class="p-6 hover:bg-gray-50 transition-colors">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center gap-3 mb-3">
                                        <h3 class="text-lg font-semibold text-gray-900">
                                            @if($shareReferral['note'])
                                                <a href="{{ route('marketplace.show', $shareReferral['note']->id) }}" class="hover:text-blue-600">
                                                    {{ $shareReferral['note']->title }}
                                                </a>
                                            @else
                                        ...
                                        @if($shareReferral['note'])
                                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">
                                                {{ currency($shareReferral['note']->price) }}
                                            </span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                                        <div class="bg-blue-50 rounded-lg p-3">
                                            ...
                                            <p class="text-lg font-bold text-blue-600">{{ number_format($shareReferral['click_count']) }}</p>
                                        </div>
                                        <div class="bg-green-50 rounded-lg p-3">
                                            <p class="text-xs text-gray-600 mb-1">Purchases</p>
                                            <p class="text-lg font-bold text-green-600">{{ number_format($shareReferral['purchase_count']) }}</p>
                                        </div>
                                        <div class="bg-purple-50 rounded-lg p-3">
                                            <p class="text-xs text-gray-600 mb-1">Commission</p>
                                            <p class="text-lg font-bold text-purple-600">{{ currency($shareReferral['total_commission_earned']) }}</p>
                                        </div>
                                        <div class="bg-yellow-50 rounded-lg p-3">
                                            <p class="text-xs text-gray-600 mb-1">Revenue</p>
                                            <p class="text-lg font-bold text-yellow-600">{{ currency($shareReferral['total_revenue_generated']) }}</p>
                                        </div>
                                    </div>

                                    <!-- Share Link -->
                                    <div class="bg-gray-50 rounded-lg p-3 flex items-center justify-between">
                                        <code class="text-xs text-gray-700 flex-1 mr-3 truncate">{{ $shareReferral['share_url'] }}</code>
                                        <button onclick="copyToClipboard('{{ $shareReferral['share_url'] }}')" 
                                    ...
                                    <!-- Purchase Details -->
                                    @if(isset($shareReferral['purchases']) && $shareReferral['purchases']->count() > 0)
                                        <div class="mt-4">
                                            <p class="text-sm font-medium text-gray-700 mb-2">Recent Purchases:</p>
                                            <div class="space-y-2">
                                                @foreach($shareReferral['purchases']->take(3) as $purchase)
                                                    <div class="flex items-center justify-between text-xs bg-gray-50 rounded p-2">
                                                        <div>
                                                            <span class="font-medium">{{ $purchase->buyer->name ?? 'Unknown' }}</span>
                                                            <span class="text-gray-500">purchased for</span>
                                                            <span class="font-semibold">{{ currency($purchase->purchase_amount) }}</span>
                                                        </div>
                                                ...
                                                @endforeach
                                                @if($shareReferral['purchases']->count() > 3)
                                                    <p class="text-xs text-gray-500 text-center">
                                                        +{{ $shareReferral['purchases']->count() - 3 }} more purchases
                                                    </p>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                <div class="ml-4 text-right">
                                    ...
                                    <p class="text-sm font-medium text-gray-900">{{ $shareReferral['created_at']->format('M d, Y') }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            @else
                <div class="p-12 text-center">
                    ...
                    <a href="{{ route('marketplace.index') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
