    ...
    <div class="py-8 sm:py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if (session('just_registered') ||
                    (auth()->user()->created_at->diffInDays(now()) <= 1 && auth()->user()->notes()->count() === 0))
                <div class="mb-6 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 rounded-xl border-2 border-blue-200 p-6 shadow-lg"
                    id="welcome-banner">
                                ...
                                <div class="flex flex-wrap gap-3">
                                    @if (auth()->user()->role === 'seller' || auth()->user()->hasRole('admin'))
                                        <a href="{{ route('notes.create') }}"
                                            class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-lg shadow-md hover:from-blue-700 hover:to-indigo-700 hover:shadow-lg transform hover:scale-105 transition-all duration-200">
                                    ...
                                    <a href="{{ route('marketplace.index') }}"
                                        class="inline-flex items-center gap-2 px-5 py-2.5 bg-white text-gray-700 font-semibold rounded-lg border-2 border-gray-300 hover:border-blue-500 hover:text-blue-600 shadow-sm hover:shadow-md transition-all duration-200">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <button
                                        onclick="document.getElementById('welcome-banner').style.display='none'; sessionStorage.setItem('welcome_banner_dismissed', 'true');"
                                        class="inline-flex items-center gap-2 px-4 py-2.5 text-gray-600 hover:text-gray-800 font-medium rounded-lg hover:bg-gray-100 transition-all duration-200">
                                        Nanti saja
                                    </button>
                                </div>
                            </div>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <div class="flex items-center gap-3">
                            <h1 class="text-3xl font-bold text-gray-900">{{ __('messages.welcome') }},
                                {{ auth()->user()->name }}!</h1>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <form action="{{ route('locale.set-currency') }}" method="POST" class="inline">
                            ...
                            <select name="currency" onchange="this.form.submit()"
                                class="text-sm rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500">
                                <option value="IDR"
                                    {{ (auth()->user()->currency ?? 'IDR') === 'IDR' ? 'selected' : '' }}>
                                    {{ __('messages.currency_option_idr') }}</option>
                                <option value="USD"
                                    {{ (auth()->user()->currency ?? 'IDR') === 'USD' ? 'selected' : '' }}>
                                    {{ __('messages.currency_option_usd') }}</option>
                                <option value="AED"
                                    {{ (auth()->user()->currency ?? 'IDR') === 'AED' ? 'selected' : '' }}>
                                    {{ __('messages.currency_option_aed') }}</option>
                                <option value="SAR"
                                    {{ (auth()->user()->currency ?? 'IDR') === 'SAR' ? 'selected' : '' }}>
                                    {{ __('messages.currency_option_sar') }}</option>
                            </select>
                        </form>
                        ...
                        <form action="{{ route('locale.set-timezone') }}" method="POST" class="inline">
                            ...
                            <select name="timezone" onchange="this.form.submit()"
                                ...
                                <option value="Asia/Jakarta"
                                    {{ (auth()->user()->timezone ?? 'Asia/Jakarta') === 'Asia/Jakarta' ? 'selected' : '' }}>
                                    {{ __('messages.timezone_option_wib') }}</option>
                                <option value="Asia/Riyadh"
                                    {{ (auth()->user()->timezone ?? 'Asia/Jakarta') === 'Asia/Riyadh' ? 'selected' : '' }}>
                                    {{ __('messages.timezone_option_ast') }}</option>
                                <option value="UTC"
                                    {{ (auth()->user()->timezone ?? 'Asia/Jakarta') === 'UTC' ? 'selected' : '' }}>
                                    {{ __('messages.timezone_option_utc') }}</option>
                            </select>
                        </form>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">
                                {{ auth()->user()->notes()->where('is_public', true)->count() }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">
                                {{ currency(auth()->user()->wallet_balance ?? 0, auth()->user()->currency) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">
                                {{ auth()->user()->transactionsAsSeller()->where('status', 'success')->count() }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                @if (auth()->user()->role === 'seller' || auth()->user()->hasRole('admin'))
                    <a href="{{ route('notes.create') }}"
                        class="bg-white overflow-hidden shadow-sm rounded-lg p-6 border border-gray-200 hover:shadow-md hover:border-blue-300 transition-all duration-200 group">
                        <div class="flex items-center">
                            <div
                                class="flex-shrink-0 bg-blue-100 rounded-lg p-3 group-hover:bg-blue-200 transition-colors duration-200">
                ...
                <a href="{{ route('marketplace.index') }}"
                    ...
                    <div class="flex items-center">
                        <div
                            class="flex-shrink-0 bg-green-100 rounded-lg p-3 group-hover:bg-green-200 transition-colors duration-200">
                            <svg class="h-6 w-6 text-green-600" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                        ...
                        <div class="ml-4">
                            <h3
                                class="text-lg font-semibold text-gray-900 group-hover:text-blue-600 transition-colors duration-200">
                                {{ __('messages.browse_marketplace') }}</h3>
                ...
                <a href="{{ route('wallet.index') }}"
                    ...
                    <div class="flex items-center">
                        <div
                            class="flex-shrink-0 bg-yellow-100 rounded-lg p-3 group-hover:bg-yellow-200 transition-colors duration-200">
                            <svg class="h-6 w-6 text-yellow-600" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex-1">
                                        <a href="{{ route('notes.show', $note) }}"
                                            class="text-base font-medium text-gray-900 hover:text-blue-600 transition-colors duration-200">
                                            {{ $note->title }}
                                        </a>
                                        ...
                                        <div class="mt-2 flex items-center gap-2">
                                            ...
                                            <span
                                                class="text-xs text-gray-500">{{ localized_diff_for_humans($note->created_at) }}</span>
                                        </div>
                                    </div>
                                    <div class="ml-4">
                                        <a href="{{ route('notes.show', $note) }}"
