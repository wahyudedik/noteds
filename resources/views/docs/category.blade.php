...
<div class="py-8 sm:py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900">{{ $categories[$category] ?? 'Documentation' }}</h1>
            <p class="mt-2 text-base text-gray-600">
                @if($category === 'wiki')
                    Comprehensive guides and information about Noteds.
        ...
        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6 mb-8">
            <form method="GET" action="{{ route('docs.category', $category) }}" class="flex gap-4">
                <input type="text" name="search" value="{{ request('search') }}" 
                    placeholder="Search in {{ $categories[$category] }}..."
        ...
        @else
            <div class="bg-white shadow-sm rounded-lg border border-gray-200 text-center py-16 px-6">
                ...
                <p class="mt-2 text-sm text-gray-500">
                    ...
                    @else
                        No {{ strtolower($categories[$category]) }} articles available yet.
                    @endif
                </p>
            </div>
        @endif
