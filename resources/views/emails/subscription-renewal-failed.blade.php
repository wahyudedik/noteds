...
<head>
    ...
    <title>{{ __('messages.subscription_renewal_failed_subject') }}</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        <h1 style="color: white; margin: 0; font-size: 24px;">{{ config('app.name', 'Noteds') }}</h1>
    </div>
    
    <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e5e7eb;">
        <h2 style="color: #dc2626; margin-top: 0; font-size: 20px;">
            ⚠️ {{ __('messages.subscription_renewal_failed_title') }}
        </h2>
        
        <p style="font-size: 16px; color: #374151;">
            {{ __('messages.subscription_renewal_failed_greeting', ['name' => $user->name]) }}
        </p>
        
        <div style="background: white; padding: 20px; border-radius: 8px; border-left: 4px solid #dc2626; margin: 20px 0;">
            <p style="margin: 0; font-size: 15px; color: #374151;">
                {{ __('messages.subscription_renewal_failed_message') }}
            </p>
        </div>
        ...
        <div style="background: #e0e7ff; padding: 15px; border-radius: 8px; margin-top: 20px;">
            <p style="margin: 0; font-size: 14px; color: #4338ca;">
                ...
                {{ __('messages.subscription_renewal_failed_note') }}
            </p>
        </div>
        ...
        <p style="font-size: 14px; color: #6b7280; text-align: center; margin: 0;">
            ...
            <a href="{{ route('contact.index') }}" style="color: #2563eb; text-decoration: none;">{{ __('messages.contact_us') }}</a>
        </p>
    </div>
    
    <div style="text-align: center; margin-top: 20px; color: #6b7280; font-size: 12px;">
        <p>{{ __('messages.email_footer', ['year' => date('Y'), 'app' => config('app.name', 'Noteds')]) }}</p>
    </div>
</body>
</html>
