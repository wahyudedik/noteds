...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
        ...
        <p>Anda telah diundang untuk bergabung dengan workspace <strong>{{ $workspace->name }}</strong> di Noteds.</p>
        
        <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #667eea;">
            <p style="margin: 0;"><strong>Workspace:</strong> {{ $workspace->name }}</p>
            <p style="margin: 5px 0 0 0;"><strong>Role:</strong> {{ $invitation->role === 'admin' ? 'Admin' : 'Member' }}</p>
            @if($workspace->description)
                <p style="margin: 10px 0 0 0; color: #6b7280;">{{ $workspace->description }}</p>
            @endif
        </div>
        
        <p>Untuk menerima undangan ini, silakan klik tombol di bawah ini dan daftar dengan email <strong>{{ $invitation->email }}</strong>:</p>
        ...
        <p style="color: #6b7280; font-size: 12px; margin-top: 30px; border-top: 1px solid #e5e7eb; padding-top: 20px;">
            <strong>Catatan:</strong> Undangan ini akan kadaluarsa pada {{ $invitation->expires_at->format('d M Y, H:i') }}.
